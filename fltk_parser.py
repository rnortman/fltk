...
class Parser:
    ...
    def parse_trivia(self, pos: int) -> fltk.fegen.pyrt.memo.ApplyResult[int, fltk_cst.Trivia] | None:
        if alt0 := self.parse_trivia__alt0(pos=pos):
            return alt0
        return None

    def apply__parse_trivia(self, pos: int) -> fltk.fegen.pyrt.memo.ApplyResult[int, fltk_cst.Trivia] | None:
        return self.packrat.apply(
            rule_callable=self.parse_trivia, rule_id=11, rule_cache=self._cache__parse_trivia, pos=pos
        )

    def parse_trivia__alt0(self, pos: int) -> fltk.fegen.pyrt.memo.ApplyResult[int, fltk_cst.Trivia] | None:
    ...
    def parse_trivia__alt0__item0(self, pos: int) -> fltk.fegen.pyrt.memo.ApplyResult[int, fltk_cst.Trivia] | None:
        result: fltk_cst.Trivia = fltk_cst.Trivia(span=fltk.fegen.pyrt.terminalsrc.Span(start=pos, end=-1))
