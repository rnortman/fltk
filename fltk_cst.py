...
class Grammar:
    ...
    children: list[tuple[typing.Optional[Label], "Rule"]] = dataclasses.field(
    ...
    def extend(
        self, children: typing.Iterable["Rule"], label: typing.Optional[Label] = None
    ) -> None:
    ...
    def child(self) -> tuple[typing.Optional[Label], "Rule"]:
        if (n := len(self.children)) != 1:
            raise ValueError(f"Expected one child but have {n}")
    ...
    def children_rule(self) -> typing.Iterator["Rule"]:
        return (
            typing.cast("Rule", child)
            for label, child in self.children
            if label == Grammar.Label.RULE
        )

    def child_rule(self) -> "Rule":
        children = list(self.children_rule())
        if (n := len(children)) != 1:
            raise ValueError(f"Expected one rule child but have {n}")
        return children[0]

    def maybe_rule(self) -> typing.Optional["Rule"]:
        children = list(self.children_rule())
        if (n := len(children)) > 1:
            raise ValueError(f"Expected at most one rule child but have {n}")
...
class Rule:
    ...
    span: fltk.fegen.pyrt.terminalsrc.Span = fltk.fegen.pyrt.terminalsrc.UnknownSpan
    children: list[
        tuple[typing.Optional[Label], typing.Union["Alternatives", "Identifier"]]
    ] = dataclasses.field(default_factory=list)

    def append(
        self,
        child: typing.Union["Alternatives", "Identifier"],
    ...
    def children_alternatives(self) -> typing.Iterator["Alternatives"]:
        return (
            typing.cast("Alternatives", child)
            for label, child in self.children
            if label == Rule.Label.ALTERNATIVES
        )

    def child_alternatives(self) -> "Alternatives":
        children = list(self.children_alternatives())
        if (n := len(children)) != 1:
            raise ValueError(f"Expected one alternatives child but have {n}")
        return children[0]

    def maybe_alternatives(self) -> typing.Optional["Alternatives"]:
        children = list(self.children_alternatives())
        if (n := len(children)) > 1:
            raise ValueError(f"Expected at most one alternatives child but have {n}")
        return children[0] if children else None
    ...
    def children_name(self) -> typing.Iterator["Identifier"]:
        return (
            ...
            for label, child in self.children
            if label == Rule.Label.NAME
        )

    def child_name(self) -> "Identifier":
        children = list(self.children_name())
        if (n := len(children)) != 1:
            raise ValueError(f"Expected one name child but have {n}")
        return children[0]

    def maybe_name(self) -> typing.Optional["Identifier"]:
        children = list(self.children_name())
        if (n := len(children)) > 1:
            raise ValueError(f"Expected at most one name child but have {n}")
        return children[0] if children else None
...
class Alternatives:
    ...
    span: fltk.fegen.pyrt.terminalsrc.Span = fltk.fegen.pyrt.terminalsrc.UnknownSpan
    children: list[tuple[typing.Optional[Label], "Items"]] = dataclasses.field(
        default_factory=list
    )
    ...
    def extend(
        self, children: typing.Iterable["Items"], label: typing.Optional[Label] = None
    ) -> None:
        self.children.extend((label, child) for child in children)
    ...
    def children_items(self) -> typing.Iterator["Items"]:
        return (
            typing.cast("Items", child)
            for label, child in self.children
            if label == Alternatives.Label.ITEMS
        )

    def child_items(self) -> "Items":
        children = list(self.children_items())
        if (n := len(children)) != 1:
            raise ValueError(f"Expected one items child but have {n}")
        return children[0]

    def maybe_items(self) -> typing.Optional["Items"]:
        children = list(self.children_items())
        if (n := len(children)) > 1:
            raise ValueError(f"Expected at most one items child but have {n}")
        return children[0] if children else None
...
class Items:
    ...
    children: list[
        tuple[
            ...
            typing.Union["Item", "fltk.fegen.pyrt.terminalsrc.Span"],
        ]
    ] = dataclasses.field(default_factory=list)

    def append(
        self,
        child: typing.Union["Item", "fltk.fegen.pyrt.terminalsrc.Span"],
        label: typing.Optional[Label] = None,
    ) -> None:
        self.children.append((label, child))
    ...
    def child(
        ...
        typing.Optional[Label], typing.Union["Item", "fltk.fegen.pyrt.terminalsrc.Span"]
    ]:
        if (n := len(self.children)) != 1:
            raise ValueError(f"Expected one child but have {n}")
        return self.children[0]
    ...
    def children_item(self) -> typing.Iterator["Item"]:
        return (
            typing.cast("Item", child)
            for label, child in self.children
            if label == Items.Label.ITEM
        )

    def child_item(self) -> "Item":
        children = list(self.children_item())
        if (n := len(children)) != 1:
            raise ValueError(f"Expected one item child but have {n}")
        return children[0]

    def maybe_item(self) -> typing.Optional["Item"]:
        children = list(self.children_item())
        if (n := len(children)) > 1:
            raise ValueError(f"Expected at most one item child but have {n}")
        return children[0] if children else None
    ...
    def extend_no_ws(
        self, children: typing.Iterable["fltk.fegen.pyrt.terminalsrc.Span"]
    ) -> None:
    ...
    def child_ws_allowed(self) -> "fltk.fegen.pyrt.terminalsrc.Span":
        ...
        if (n := len(children)) != 1:
            raise ValueError(f"Expected one ws_allowed child but have {n}")
        return children[0]

    def maybe_ws_allowed(self) -> typing.Optional["fltk.fegen.pyrt.terminalsrc.Span"]:
        children = list(self.children_ws_allowed())
        if (n := len(children)) > 1:
            raise ValueError(f"Expected at most one ws_allowed child but have {n}")
        return children[0] if children else None
    ...
    def child_ws_required(self) -> "fltk.fegen.pyrt.terminalsrc.Span":
        ...
        if (n := len(children)) != 1:
            raise ValueError(f"Expected one ws_required child but have {n}")
        return children[0]

    def maybe_ws_required(self) -> typing.Optional["fltk.fegen.pyrt.terminalsrc.Span"]:
        children = list(self.children_ws_required())
        if (n := len(children)) > 1:
            raise ValueError(f"Expected at most one ws_required child but have {n}")
        return children[0] if children else None
...
class Item:
    ...
    children: list[
        tuple[
            typing.Optional[Label],
            typing.Union["Disposition", "Identifier", "Quantifier", "Term"],
        ]
    ] = dataclasses.field(default_factory=list)
    ...
    def children_disposition(self) -> typing.Iterator["Disposition"]:
        return (
            typing.cast("Disposition", child)
            for label, child in self.children
            if label == Item.Label.DISPOSITION
        )

    def child_disposition(self) -> "Disposition":
        children = list(self.children_disposition())
        if (n := len(children)) != 1:
            raise ValueError(f"Expected one disposition child but have {n}")
        return children[0]

    def maybe_disposition(self) -> typing.Optional["Disposition"]:
        children = list(self.children_disposition())
        if (n := len(children)) > 1:
            raise ValueError(f"Expected at most one disposition child but have {n}")
        return children[0] if children else None
    ...
    def child_label(self) -> "Identifier":
        ...
        if (n := len(children)) != 1:
            raise ValueError(f"Expected one label child but have {n}")
        return children[0]

    def maybe_label(self) -> typing.Optional["Identifier"]:
        children = list(self.children_label())
        if (n := len(children)) > 1:
            raise ValueError(f"Expected at most one label child but have {n}")
        return children[0] if children else None
    ...
    def children_quantifier(self) -> typing.Iterator["Quantifier"]:
        return (
            typing.cast("Quantifier", child)
            for label, child in self.children
            if label == Item.Label.QUANTIFIER
        )

    def child_quantifier(self) -> "Quantifier":
        children = list(self.children_quantifier())
        if (n := len(children)) != 1:
            raise ValueError(f"Expected one quantifier child but have {n}")
        return children[0]

    def maybe_quantifier(self) -> typing.Optional["Quantifier"]:
        children = list(self.children_quantifier())
        if (n := len(children)) > 1:
            raise ValueError(f"Expected at most one quantifier child but have {n}")
        return children[0] if children else None
    ...
    def children_term(self) -> typing.Iterator["Term"]:
        return (
            typing.cast("Term", child)
            for label, child in self.children
            if label == Item.Label.TERM
        )

    def child_term(self) -> "Term":
        children = list(self.children_term())
        if (n := len(children)) != 1:
            raise ValueError(f"Expected one term child but have {n}")
        return children[0]

    def maybe_term(self) -> typing.Optional["Term"]:
        children = list(self.children_term())
        if (n := len(children)) > 1:
            raise ValueError(f"Expected at most one term child but have {n}")
        return children[0] if children else None
...
class Term:
    ...
    children: list[
        tuple[
            typing.Optional[Label],
            typing.Union["Alternatives", "Identifier", "Literal", "RawString"],
        ]
    ] = dataclasses.field(default_factory=list)
    ...
    def children_alternatives(self) -> typing.Iterator["Alternatives"]:
        return (
            typing.cast("Alternatives", child)
            for label, child in self.children
            if label == Term.Label.ALTERNATIVES
        )
    ...
    def children_identifier(self) -> typing.Iterator["Identifier"]:
        return (
            typing.cast("Identifier", child)
            for label, child in self.children
            if label == Term.Label.IDENTIFIER
        )

    def child_identifier(self) -> "Identifier":
        children = list(self.children_identifier())
        if (n := len(children)) != 1:
            raise ValueError(f"Expected one identifier child but have {n}")
        return children[0]

    def maybe_identifier(self) -> typing.Optional["Identifier"]:
        children = list(self.children_identifier())
        if (n := len(children)) > 1:
            raise ValueError(f"Expected at most one identifier child but have {n}")
        return children[0] if children else None
    ...
    def children_literal(self) -> typing.Iterator["Literal"]:
        return (
            typing.cast("Literal", child)
            for label, child in self.children
            if label == Term.Label.LITERAL
        )

    def child_literal(self) -> "Literal":
        children = list(self.children_literal())
        if (n := len(children)) != 1:
            raise ValueError(f"Expected one literal child but have {n}")
        return children[0]

    def maybe_literal(self) -> typing.Optional["Literal"]:
        children = list(self.children_literal())
        if (n := len(children)) > 1:
            raise ValueError(f"Expected at most one literal child but have {n}")
        return children[0] if children else None
    ...
    def children_regex(self) -> typing.Iterator["RawString"]:
        return (
            typing.cast("RawString", child)
            for label, child in self.children
            if label == Term.Label.REGEX
        )

    def child_regex(self) -> "RawString":
        children = list(self.children_regex())
        if (n := len(children)) != 1:
            raise ValueError(f"Expected one regex child but have {n}")
        return children[0]

    def maybe_regex(self) -> typing.Optional["RawString"]:
        children = list(self.children_regex())
        if (n := len(children)) > 1:
            raise ValueError(f"Expected at most one regex child but have {n}")
        return children[0] if children else None
...
class Disposition:
    ...
    span: fltk.fegen.pyrt.terminalsrc.Span = fltk.fegen.pyrt.terminalsrc.UnknownSpan
    children: list[
        tuple[typing.Optional[Label], "fltk.fegen.pyrt.terminalsrc.Span"]
    ] = dataclasses.field(default_factory=list)

    def append(
        self,
        child: "fltk.fegen.pyrt.terminalsrc.Span",
        label: typing.Optional[Label] = None,
    ) -> None:
        self.children.append((label, child))
    ...
    def extend(
        self,
        children: typing.Iterable["fltk.fegen.pyrt.terminalsrc.Span"],
        label: typing.Optional[Label] = None,
    ) -> None:
        self.children.extend((label, child) for child in children)
    ...
    def child_include(self) -> "fltk.fegen.pyrt.terminalsrc.Span":
        ...
        if (n := len(children)) != 1:
            raise ValueError(f"Expected one include child but have {n}")
        return children[0]

    def maybe_include(self) -> typing.Optional["fltk.fegen.pyrt.terminalsrc.Span"]:
        children = list(self.children_include())
        if (n := len(children)) > 1:
            raise ValueError(f"Expected at most one include child but have {n}")
        return children[0] if children else None
    ...
    def child_inline(self) -> "fltk.fegen.pyrt.terminalsrc.Span":
        ...
        if (n := len(children)) != 1:
            raise ValueError(f"Expected one inline child but have {n}")
        return children[0]

    def maybe_inline(self) -> typing.Optional["fltk.fegen.pyrt.terminalsrc.Span"]:
        children = list(self.children_inline())
        if (n := len(children)) > 1:
            raise ValueError(f"Expected at most one inline child but have {n}")
        return children[0] if children else None
    ...
    def child_suppress(self) -> "fltk.fegen.pyrt.terminalsrc.Span":
        ...
        if (n := len(children)) != 1:
            raise ValueError(f"Expected one suppress child but have {n}")
        return children[0]

    def maybe_suppress(self) -> typing.Optional["fltk.fegen.pyrt.terminalsrc.Span"]:
        children = list(self.children_suppress())
        if (n := len(children)) > 1:
            raise ValueError(f"Expected at most one suppress child but have {n}")
        return children[0] if children else None
...
class Quantifier:
    ...
    def extend_one_or_more(
        self, children: typing.Iterable["fltk.fegen.pyrt.terminalsrc.Span"]
    ) -> None:
        self.children.extend(
            (Quantifier.Label.ONE_OR_MORE, child) for child in children
        )

    def children_one_or_more(
        self,
    ) -> typing.Iterator["fltk.fegen.pyrt.terminalsrc.Span"]:
        return (
            typing.cast("fltk.fegen.pyrt.terminalsrc.Span", child)
    ...
    def child_one_or_more(self) -> "fltk.fegen.pyrt.terminalsrc.Span":
        ...
        if (n := len(children)) != 1:
            raise ValueError(f"Expected one one_or_more child but have {n}")
        return children[0]

    def maybe_one_or_more(self) -> typing.Optional["fltk.fegen.pyrt.terminalsrc.Span"]:
        children = list(self.children_one_or_more())
        if (n := len(children)) > 1:
            raise ValueError(f"Expected at most one one_or_more child but have {n}")
        return children[0] if children else None
    ...
    def child_optional(self) -> "fltk.fegen.pyrt.terminalsrc.Span":
        ...
        if (n := len(children)) != 1:
            raise ValueError(f"Expected one optional child but have {n}")
        return children[0]

    def maybe_optional(self) -> typing.Optional["fltk.fegen.pyrt.terminalsrc.Span"]:
        children = list(self.children_optional())
        if (n := len(children)) > 1:
            raise ValueError(f"Expected at most one optional child but have {n}")
        return children[0] if children else None
    ...
    def extend_zero_or_more(
        self, children: typing.Iterable["fltk.fegen.pyrt.terminalsrc.Span"]
    ) -> None:
        self.children.extend(
            (Quantifier.Label.ZERO_OR_MORE, child) for child in children
        )

    def children_zero_or_more(
        self,
    ) -> typing.Iterator["fltk.fegen.pyrt.terminalsrc.Span"]:
        return (
            typing.cast("fltk.fegen.pyrt.terminalsrc.Span", child)
    ...
    def child_zero_or_more(self) -> "fltk.fegen.pyrt.terminalsrc.Span":
        ...
        if (n := len(children)) != 1:
            raise ValueError(f"Expected one zero_or_more child but have {n}")
        return children[0]

    def maybe_zero_or_more(self) -> typing.Optional["fltk.fegen.pyrt.terminalsrc.Span"]:
        children = list(self.children_zero_or_more())
        if (n := len(children)) > 1:
            raise ValueError(f"Expected at most one zero_or_more child but have {n}")
        return children[0] if children else None
...
class RawString:
    ...
    def child_value(self) -> "fltk.fegen.pyrt.terminalsrc.Span":
        ...
        if (n := len(children)) != 1:
            raise ValueError(f"Expected one value child but have {n}")
        return children[0]

    def maybe_value(self) -> typing.Optional["fltk.fegen.pyrt.terminalsrc.Span"]:
        children = list(self.children_value())
        if (n := len(children)) > 1:
            raise ValueError(f"Expected at most one value child but have {n}")
        return children[0] if children else None
