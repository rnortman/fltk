...
Nothing generated is in git except the seed. `fltk._native`'s crate root comes from
`//:native_lib_rs` (`generate_rust_lib`), which `//:native_crate_srcs` flattens into one
directory with the hand-written `src/span.rs` so `mod span;` resolves against the crate root.
The `.rs` for each in-tree crate comes from the `generate_rust_parser`
...
Bazel is the build system, and the only one. There is no uv, no maturin and no cargo: no manifest is tracked, nothing in `make check` or CI runs `cargo`, and `tests/test_cargo_retirement.py` is what keeps it that way (no tracked `Cargo.toml` / `Cargo.lock` / `deny.toml` / `rust-toolchain.toml`, no `cargo` invocation in the Makefile, the CI workflow, `.bazelrc`, `.claude/settings.json` or the repo's Starlark, no `requires-cargo` tag). Every crate has Bazel targets in both feature flavors, so `bazel test //...` compiles and runs them and the clippy aspect lints them, over strictly more configurations than the retired cargo lanes covered. Third-party crates come from crate_universe in `from_specs` form — one `crate.spec` tag per direct dependency in `MODULE.bazel`, no manifest to read.

`make check` is four steps, three of them Bazel lanes and one a lock diff. Every step prints a start line and its wall time in both modes — a failing step reports its duration and the gate total too — and the two lanes whose names no longer partition the work say so on those lines: `bazel-test` builds the lint surface as well, which is what makes `bazel-lint`'s seconds a cache-hit confirmation rather than the cost of linting. `VERBOSE=1` additionally streams each step's output as it runs instead of buffering it and printing only on failure. Verbose is the default in CI (GitHub Actions sets `CI=true`) and quiet locally; an explicit `VERBOSE=0`/`VERBOSE=1` overrides in either direction.

- `bazel-toolchain-guard` — the MODULE.bazel mirror check below.
- `make bazel-test` (`bazel test --config lint //...` plus the no-pyo3 cquery guard) covers fltk's own internal Bazel surface: the `tests/bazel_rules` analysis tests, the codegen smoke targets, the `:native` cdylib, every pytest file as its own `py_test`, and the `rust_test` targets that run each runtime crate's unit tests on its `:no_python` flavor — plus the feature carve-outs nothing else covers: `//crates/fltk-ast-core:no_features_test` (every feature off), `//crates/fltk-cst-core:python_test` (the `python` feature set, including the pyclass ABI-layout probes), and `//crates/fegen-rust:native_parser_test` / `//crates/fltkfmt:cli_test`, which build their crates from Bazel-generated sources. The three fixture extensions under `tests/rust_poc_cst`, `tests/rust_cst_fixture` and `tests/rust_parser_fixture` are Bazel targets too, each hand-assembled from its own codegen targets rather than through `fltk_pyo3_cdylib`. The cquery guard asserts pyo3 is absent from every `:no_python` target's graph, every `rust_binary`'s, and `//tests:rust_gate_lib`'s (which matches neither pattern); it is one union cquery over all of them (`deps(set(...))` is the union of the members' `deps`), with a per-target loop that runs only to attribute a failure. The positive control that keeps that negative assertion from passing vacuously stays per target, through one loading-phase `bazel query "rdeps(set(...), //crates/fltk-cst-core:no_python)"` whose output must name every derived target — a `cquery` there costs ~25s (reverse-dep inversion over the configured graph) to prove a structural fact. Since `query` unions every `select()` branch, the per-target control is an unconfigured fact and no per-target *configured* fact is asserted; that residual is accepted and stated in the recipe's comment. Both this lane's `bazel test` and its cqueries carry `--config lint`, the same configuration `bazel-lint` builds under: alternating configurations mid-gate discards Bazel's in-memory analysis cache and cost the gate tens of seconds on every hot run. The cost of that uniformity is the assertion surface: the root lane's pyo3 check holds at `--//bzl:lint=true`, not at the default configuration a downstream consumer builds, so a pyo3 edge behind a `select()` on the lint setting would be invisible to it — `TODO(pyo3-guard-shared-helper)` carries the cheap restoration. The gate's configuration therefore differs from a bare `bazel test //...`, so a dev run and a gate run still thrash each other's analysis cache — type `bazel test --config lint //...` to stay on the gate's.
...
The remaining step is `check-bazel-locks`, the lock diff. `docs/bazel-consumer-guide.md` is the consumer-facing recipe book (one recipe per supported configuration, the `:no_python` runtime targets, the one-serde rule for serde-mode pure-Rust consumers, and the no-pyo3 verification queries); keep it in step with `rust.bzl` and the crate BUILD files.

**No host Rust toolchain is needed**: rules_rust downloads its own, and nothing outside Bazel compiles Rust here. A fresh clone needs Bazel (via `.bazelversion`/bazelisk) and nothing else — no rustup, no `cargo fetch`.

The single Rust version pin is the root `MODULE.bazel`'s `rust.toolchain(versions = [...])` tag. `make check` denies all clippy warnings, so an unpinned toolchain would let a new stable release break CI on an unchanged commit. Every other `MODULE.bazel` that pulls in rules_rust must mirror that tag, or it silently compiles the same sources with rules_rust's default toolchain; `make bazel-toolchain-guard` — the first `CHECK_STEPS` entry and a prerequisite of all three Bazel lanes — derives the mirror list from the tracked `MODULE.bazel` files and fails if any of them is stale or missing the tag, so a newly added Bazel workspace cannot silently opt out of the pin. Bumping the pin is a deliberate manual change (Dependabot does not understand it): edit the `versions` line in every `MODULE.bazel`, run `make check`, and fix any newly-surfaced lints in the same commit.
...
No `Cargo.lock` and no `Cargo.toml` is tracked. Third-party Rust dependencies are `crate.spec` tags in each workspace's `MODULE.bazel`, and crate_universe writes the resulting pins into two files per workspace: `cargo-bazel-lock.json` (the rendered hub) and `cargo-bazel-resolved.lock` (the Cargo-lock-format resolution it renders from — cargo-bazel's own file, not a cargo input). Never hand-edit these files or add a second generator (a pip updater, another exporter writing `requirements_lock.txt`) — its edits would be silently clobbered on the next regeneration.

Six tracked locks are Bazel-written: the two `MODULE.bazel.lock` files (repo root and `tests/bazel_consumer/`) and the crate_universe pair in each workspace. bzlmod's default `lockfile_mode` is `update` and a repin rewrites the crate_universe pair the same way, so a Bazel run repairs a stale lock in place and still reports green — a manifest edit can leave the committed lock wrong indefinitely. `make check-bazel-locks` is the diff that forces the repair to be committed, and it runs last in `CHECK_STEPS` — after `bazel-test` and `bazel-consumer-check`, which are the lanes that do the repairing. Same caveat as above: the diff step alone passes vacuously with no prior Bazel run, so `make check` is the gate that clears them.

**`CARGO_BAZEL_REPIN=1` after editing a `crate.spec`.** With an already-populated Bazel output base, a Bazel lane can fail with `no such target '@fltk_crates//:<crate>'` — the crate_universe repos were resolved from the previous specs. One run with `CARGO_BAZEL_REPIN=1` (e.g. `CARGO_BAZEL_REPIN=1 bazel test //...`) re-resolves them, rewrites both crate_universe locks (commit them) and clears it. The repin runs the cargo binary inside rules_rust's hermetic toolchain, not a host one. A fresh cache, including CI, never sees this.

Rust dependency bumps are manual — edit the spec version, repin, `make check` — and there is no advisory or license gate over them anymore; `TODO(rust-supply-chain-audit)` carries the successor.
...
No pytest target compiles Rust. "Does the generated Rust compile, and what does it do when it runs" is Bazel's job: `tests/rust_gate_cases.py`'s `CASES` is generated into one crate by `//tests:rust_gate_srcs`, `//tests:rust_gate_lib` compiles it, `//tests:rust_gate_runtime_test` runs the `#[test]` module each case carries, and `//tests:rust_gate_build_test` covers the compile-only cases. `//tests:prelude_python_probe` is the same shadowing-grammar `cst.rs` at the `python` feature set with pyo3 linked, which is the half the gate crate's featureless build compiles out. Clippy over all of them comes from the lint aspect, in the same lane as every other crate's. The declared output list in `tests/BUILD.bazel` is what a Bazel action needs to name its outputs statically, and `tests/test_rust_gate_manifest.py` holds that list to what `CASES` implies.
