...
The cross-backend comparison must work **without either backend importing the other**. Mechanism: a duck-typed sentinel. Each label/kind type carries a way to produce its operand's canonical name that any backend can invoke reflectively. Two concrete options, both validated:

- **Python side:** a class-level marker (e.g. a `_fltk_canonical` staticmethod, or a `canonical_name` property — see Open question §6.1) that maps a member to its string. `__eq__` reads `getattr(type(other), "_fltk_canonical", None)`; if absent → `NotImplemented`; else compare strings.
- **Rust side:** the `__repr__` string is already the canonical name; the hand-written `__eq__` reads the operand's canonical name via the same duck-typed marker (see §2.3 for why a dedicated marker is preferred over `repr`).
...
- a per-member canonical-name source. Cleanest: a class attribute mapping member name → `"<ClassName>.Label.<NAME>"`, or a method computing it from `self.name` + a class-level `_fltk_class_name`. (Generator already knows `class_name` at `gsm2tree.py:112`.)
- `__eq__(self, other)`: **same-type fast path first** — `if other is self: return True`; then `if type(other) is type(self): return self.name == other.name` (member-name compare, no canonical-string build), mirroring the Rust own-type fast path (§2.3). This keeps the hot same-backend `children_X` filter (`label == Class.Label.X`, run per child — `gsm2tree.py:201`) off the string-building path, honoring the `requirements.md:126` performance SHOULD ("avoid per-comparison object allocation or string formatting on the same-backend path"). Only the cross-type case falls through to the canonical-string compare: if `other` exposes the canonical marker → compare canonical strings (returns `True`/`False`); else return `NotImplemented`. **Must** return `NotImplemented` (not `False`) for foreign operands so Python invokes the reflected Rust `__eq__` and the `py == rust` direction can reach `True` (`requirements.md:122`). For the equal case at least one side returns `True` as a value — the Python side does (`requirements.md:69` caveat).
...
**Canonical-name extraction from a foreign Rust label:** prefer a dedicated marker method over `repr()` parsing — `repr()` for nodes is `ClassName(span=..., children=...)` (`gsm2tree_rs.py:458-464`), so a bare `repr`-based key would be ambiguous if ever applied beyond labels. The label `__repr__` *is* the canonical string today, but the design pins the key to an explicit marker (method/attr that both backends expose) so the contract is not accidentally coupled to `repr` formatting (resolves the §6.2 repr question independently).
...
- **Per-node discriminant:** every node carries `kind`. 
  - Python concrete (`fltk_cst.py`): a class-level `kind: typing.ClassVar[...] = NodeKind.<Rule>` (or instance attr); the Protocol declares `kind: typing.Literal[NodeKind.<Rule>]` so unions narrow.
...
A non-label object defining the same marker attribute would be treated as a label. **Mitigation:** marker name is namespaced/unlikely (`_fltk_canonical`) and returns a string; mismatched strings simply compare unequal. No raise, worst case a `False`. Acceptable; documented.
...
1. **`canonical_name` public property.** Expose `label.canonical_name` / `kind.canonical_name` as committed API, or keep the marker internal (`requirements.md:132`)? **Decision (settled): internal marker only** (Option A). The marker must exist regardless; this is purely whether it is a *documented* surface. Redirect: user says "expose `.canonical_name`" → make the marker a public property on both backends and both enum families.
