...
def _escape_source_name(s: str) -> str:
    """Escape a source name for use inside a Rust doc-comment backtick span.

    Replaces backslashes (path separators on Windows) and backtick characters so
...
class RustCstGenerator:
    ...
    def __init__(self, grammar: gsm.Grammar, source_name: str | None = None):
    ...
    def generate_protocol(self) -> str:
        ...
        The returned text is byte-identical to the Python ``generate --protocol`` output for
