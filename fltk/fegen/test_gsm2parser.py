...
def test_single() -> None:
    ...
    LITERAL: Final = "as'\\\"df"  # Make sure escaping works well
    item = gsm.Item(
        label="testlabel",
    ...
    method = pgen.parser_class.block.get_leaf_scope().lookup(
        name=parser_info.apply_name, recursive=False
    )
    ...
    locals = dict(
        ApplyResult=memo.ApplyResult, Span=terminalsrc.Span, Optional=Optional
    )
    exec(mod, locals)
    LOG.info(locals.keys())
    item_parser = locals[method.name]
    assert callable(item_parser)

    parser = mock.Mock(spec_set=["consume_literal"])
    EXPECT: Final = memo.ApplyResult(42, terminalsrc.Span(0, 42))
    ...
    assert item_parser(parser, 11) == None
...
def test_bootstrap() -> None:
    pgen = g2p.ParserGenerator(
        ...
        cstgen=gsm2tree.CstGenerator(
            grammar=bootstrap.grammar, py_module=pyreg.Builtins
        ),
    )
    LOG.info(pgen)
    LOG.info(pgen.parser_class)
