...
class RustParserGenerator:
    ...
    def _gen_header(self) -> str:
        ...
        # CST module import (shared helper; mirrored by gsm2unparser_rs._gen_header).
    ...
    def _gen_constants(self) -> str:
        ...
        n = len(rule_names)
        names_lit = ", ".join(f'"{rust_str_lit(name)}"' for name in rule_names)
        ...
        if self._regex_patterns:
            ...
            patterns_lit = ", ".join(f'"{rust_str_lit(p)}"' for p in self._regex_patterns)
    ...
    def _gen_parser_struct(self) -> str:
        ...
        lines.append("    pub fn new(text: &str, filename: Option<&str>, capture_trivia: bool) -> Self {")
        lines.append("        Self::from_source_text(SourceText::from_str(text, filename), capture_trivia)")
        lines.append("    }")
        lines.append("")
    ...
    def _gen_consume_term(
        ...
        if isinstance(term, gsm.Identifier):
            ...
            escaped = rust_str_lit(term.value)
