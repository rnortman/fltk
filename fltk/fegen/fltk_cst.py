...
class Grammar:
    ...
    def _check_child_type_for_mutators(self, child: _cstp.Rule | _cstp.Trivia) -> Rule | Trivia:
        if isinstance(child, Rule | Trivia):
            return child
    ...
    def _children_snapshot(self, label: Grammar.Label) -> list[Rule | Trivia]:
    ...
    def children_rule(self) -> typing.Iterator[Rule]:
        return iter(typing.cast("list[Rule]", self._children_snapshot(Grammar.Label.RULE)))

    def child_rule(self) -> Rule:
        children = typing.cast("list[Rule]", self._children_snapshot(Grammar.Label.RULE))
        if (n := len(children)) != 1:
    ...
    def rule(self) -> list[Rule]:
        return typing.cast("list[Rule]", self._children_snapshot(Grammar.Label.RULE))
...
class Items:
    ...
    _LABELS_BY_CANONICAL_NAME = types.MappingProxyType(
        {
            ...
            "Items.Label.NO_WS": Label.NO_WS,
            "Items.Label.WS_ALLOWED": Label.WS_ALLOWED,
            "Items.Label.WS_REQUIRED": Label.WS_REQUIRED,
        }
    )
    ...
    def append_ws_allowed(self, child: fltk.fegen.pyrt.span_protocol.SpanProtocol) -> None:
        self.children.append((Items.Label.WS_ALLOWED, self._check_child_type_for_mutators(child)))

    def extend_ws_allowed(self, children: typing.Iterable[fltk.fegen.pyrt.span_protocol.SpanProtocol]) -> None:
        self.children.extend(
            [(Items.Label.WS_ALLOWED, self._check_child_type_for_mutators(child)) for child in children]
        )

    def children_ws_allowed(self) -> typing.Iterator[fltk.fegen.pyrt.span_protocol.SpanProtocol]:
        return iter(
            typing.cast(
                "list[fltk.fegen.pyrt.span_protocol.SpanProtocol]", self._children_snapshot(Items.Label.WS_ALLOWED)
            )
        )
    ...
    def append_ws_required(self, child: fltk.fegen.pyrt.span_protocol.SpanProtocol) -> None:
        self.children.append((Items.Label.WS_REQUIRED, self._check_child_type_for_mutators(child)))

    def extend_ws_required(self, children: typing.Iterable[fltk.fegen.pyrt.span_protocol.SpanProtocol]) -> None:
        self.children.extend(
            [(Items.Label.WS_REQUIRED, self._check_child_type_for_mutators(child)) for child in children]
        )

    def children_ws_required(self) -> typing.Iterator[fltk.fegen.pyrt.span_protocol.SpanProtocol]:
        return iter(
            typing.cast(
                "list[fltk.fegen.pyrt.span_protocol.SpanProtocol]", self._children_snapshot(Items.Label.WS_REQUIRED)
            )
        )
...
class Trivia:
    ...
    _LABELS_BY_CANONICAL_NAME = types.MappingProxyType(
        {"Trivia.Label.BLOCK_COMMENT": Label.BLOCK_COMMENT, "Trivia.Label.LINE_COMMENT": Label.LINE_COMMENT}
    )
    ...
    def _check_child_type_for_mutators(
        ...
        if isinstance(child, BlockComment | LineComment | fltk.fegen.pyrt.terminalsrc.Span):
            return child
        _ns = _get_native_span_type()
        if _ns is not None and isinstance(child, _ns):
            native_span: typing.Any = child
    ...
    def _children_snapshot(
        self, label: Trivia.Label
    ) -> list[BlockComment | LineComment | fltk.fegen.pyrt.span_protocol.SpanProtocol]:
        return [child for (lbl, child) in self.children if lbl == label]
    ...
    def block_comment(self) -> list[BlockComment]:
        return typing.cast("list[BlockComment]", self._children_snapshot(Trivia.Label.BLOCK_COMMENT))
...
class LineComment:
    ...
    _LABELS_BY_CANONICAL_NAME = types.MappingProxyType(
        {"LineComment.Label.CONTENT": Label.CONTENT, "LineComment.Label.PREFIX": Label.PREFIX}
    )
